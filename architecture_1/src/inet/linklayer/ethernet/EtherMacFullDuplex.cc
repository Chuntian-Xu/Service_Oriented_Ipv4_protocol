// inet/src/inet/linklayer/ethernet/EtherMacFullDuplex.cc
...
{
    EV_INFO << "!!! --> EtherMacFullDuplex::handleMessageWhenUp(cMessage *msg)" <<endl;  // new added
    if (channelsDiffer)
...
{
    EV_INFO<<"!!! --> EtherMacFullDuplex::handleSelfMessage(cMessage *msg)"<<endl;  // new added
...
{
    EV_INFO << "!!! --> EtherMacFullDuplex::startFrameTransmission()"<<endl; // new added
...
{
    EV_INFO << "!!! --> EtherMacFullDuplex::handleUpperPacket(Packet *packet) " <<endl;  // new added
    ...
    EV_INFO<<"    --> Src MacAddress: "<<frame->getSrc()<<endl; // new added
    EV_INFO<<"    --> Dest MacAddress: "<<frame->getDest()<<endl; // new added
    ...
    if (packet->getDataLength() > MAX_ETHERNET_FRAME_BYTES) {    //FIXME two MAX FRAME BYTES in specif...
        throw cRuntimeError("packet from higher layer (length=%s) exceeds maximum Ethernet frame size (%s)",
                B(packet->getByteLength()).str().c_str(), B(MAX_ETHERNET_FRAME_BYTES).str().c_str());
    }
    ...
    if (frame->getSrc().isUnspecified()) {
        ...
        frame = newFrame;
        EV_INFO<<"    --> Src MacAddress = getMacAddress(): "<<frame->getSrc()<<endl; // new added
    }
...
{
    EV_INFO<<"!!! --> void EtherMacFullDuplex::processMsgFromNetwork(EthernetSignal *signal)"<<"\n";   // new added
    ...
    // parse packet's MacHeader，and check if MACaddress matches local Macaddress new added
...
{
    EV_INFO<<"!!! --> EtherMacFullDuplex::handleEndIFGPeriod()"<<endl;
...
{
    EV_INFO<<"!! --> EtherMacFullDuplex::handleEndTxPeriod()"<<endl;  // new added
...
{
    EV_INFO << "!!! --> void EtherMacFullDuplex::processReceivedDataFrame(Packet *packet, const Ptr<const EthernetMacHeader>& frame)" <<endl;  // new added
    // statistics
...
{
    EV_INFO<<"!!! --> EtherMacFullDuplex::scheduleEndIFGPeriod()"<<endl;  // new added
    ASSERT(nullptr == currentTxFrame);
