...
//    EV_INFO<<"!!!!!! --> L3AddressResolver::resolve(const char *s, int addrType):  s: "<<s<<" addrType: "<<addrType<<"\n"; // new added
    L3Address addr;
    if (!tryResolve(s, addr, addrType)) throw cRuntimeError("L3AddressResolver: address `%s' not configured (yet?)", s);
    return addr;
}
...
bool L3AddressResolver::tryParse(L3Address& result, const char *addr, int addrType) {
    EV_INFO<<"!!! --> L3AddressResolver::tryParse(L3Address& result, const char *addr, int addrType):  addr: "<<addr<<" addrType: "<< addrType <<"\n"; // new added
    Ipv6Address ipv6;
    MacAddress mac;
...
bool L3AddressResolver::tryResolve(const char *s, L3Address& result, int addrType) {
    EV_INFO<<"!!! --> L3AddressResolver::tryResolve(const char *s, L3Address& result, int addrType):  s:"<<s<<" addrType: "<<addrType<<"\n"; // new added
    // empty address
    result = L3Address();
    if (!s || !*s) return true;

    // handle address literal
    if (tryParse(result, s, addrType)) return true;
    ...
    if (c) throw cRuntimeError("L3AddressResolver: syntax error parsing address spec `%s'", s);

    // find module
    cModule *mod = getSimulation()->getModuleByPath(modname.c_str());
    EV_INFO<<"mod: "<<mod->getFullName() <<"\n";
    if (!mod) throw cRuntimeError("L3AddressResolver: module `%s' not found", modname.c_str());
    ...
    if (!destnodename.empty()) {
        ...
        if (!destnode) throw cRuntimeError("L3AddressResolver: destination module `%s' not found", destnodename.c_str());
        result = addressOf(mod, destnode, addrType);
    }
    else if (ifname.empty()) result = addressOf(mod, addrType);
    else if (ifname == "routerId") result = routerIdOf(mod); // addrType is meaningless here, routerId is protocol independent
    else result = addressOf(mod, ifname.c_str(), addrType);
...
// new added
IIpv4SidTable *L3AddressResolver::findIpv4SidTableOf(cModule *host) {
#ifdef WITH_IPv4
    return dynamic_cast<IIpv4SidTable *>(host->getModuleByPath(".ipv4.sidTable"));
#else // ifdef WITH_IPv4
    return nullptr;
#endif // ifdef WITH_IPv4
}
...
// new added
IIpv4CidshoreTable *L3AddressResolver::findIpv4CidshoreTableOf(cModule *host) {
#ifdef WITH_IPv4
    return dynamic_cast<IIpv4CidshoreTable *>(host->getModuleByPath(".ipv4.cidshoreTable"));
#else // ifdef WITH_IPv4
    return nullptr;
#endif // ifdef WITH_IPv4
}
