...
{
//    EV_INFO<<"!!! --> *MessageDispatcher::handlePacket(Packet *packet, cGate *inGate)"<<endl;  // new added
    auto socketInd = packet->findTag<SocketInd>();
    if (socketInd != nullptr) {
//        EV_INFO<<"    --> socketInd != nullptr"<<endl;  // new added
...
//        EV_INFO<<"    --> dispatchProtocolReq != nullptr"<<endl;  // new added
...
//            EV_INFO<<"    --> servicePrimitive == static_cast<ServicePrimitive>(-1)"<<endl;  // new added
//            EV<<"    --> dispatchProtocolReq->getProtocol()"<<dispatchProtocolReq->getProtocol()->str()<<endl;  // new added
            if (packetProtocolTag != nullptr && dispatchProtocolReq->getProtocol() == packetProtocolTag->getProtocol()){
//                EV<<"    --> packetProtocolTag->getProtocol()"<<packetProtocolTag->getProtocol()->str()<<endl;  // new added
//                EV_INFO<<"    --> servicePrimitive = SP_INDICATION"<<endl;  // new added
                servicePrimitive = SP_INDICATION;
            }
            else {
//                EV_INFO<<"    --> servicePrimitive = SP_REQUEST"<<endl;  // new added
