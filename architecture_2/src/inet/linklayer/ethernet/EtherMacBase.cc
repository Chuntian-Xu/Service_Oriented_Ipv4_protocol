// inet/src/inet/linklayer/ethernet/EtherMacBase.cc
...
bool EtherMacBase::dropFrameNotForUs(Packet *packet, const Ptr<const EthernetMacHeader>& frame)
{
    ...
    // in promiscuous mode.
    EV_INFO<<"    --> dest mac address: "<<frame->getDest()<<endl;  // new added
    if (frame->getDest().equals(getMacAddress())) {
        EV_WARN << "Frame `" << packet->getName() << "' destined to us, do not discarding\n"; // new added
        return false;
    }
...
{
    EV_INFO<<"!!! --> EtherMacBase::changeTransmissionState(MacTransmitState newState)"<<endl;  // new added
