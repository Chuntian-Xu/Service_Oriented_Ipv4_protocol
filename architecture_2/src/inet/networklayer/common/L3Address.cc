...
//    EV_INFO<<"!!! --> L3Address::get(AddressType type) const: "<<"lo: "<<lo<<" hi: "<<hi<<"\n"; // new added
    if (getType() == type) return lo;
    else throw cRuntimeError("L3Address::get(AddressType type): Address is not of the given type");
}
...
//    EV_INFO<<"!!! --> L3Address::AddressType L3Address::getType() const: (AddressType)(hi & 0xFF): "<<(AddressType)(hi & 0xFF)<<"\n";
    if (hi >> 48 == RESERVED_IPV6_ADDRESS_RANGE) return (AddressType)(hi & 0xFF);
    else return L3Address::IPv6;
}

IL3AddressType *L3Address::getAddressType() const {
    EV_INFO<<"IL3AddressType *L3Address::getAddressType() const "<<"\n";
    switch (getType()) {
        case L3Address::NONE:
...
ClnsAddress L3Address::toClns() const
{
    switch (getType()) {
        ...
        default:
            throw cRuntimeError("L3Address::toClns(): Address is not of the given type");
    }
}
