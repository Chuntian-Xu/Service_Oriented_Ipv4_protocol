...
void Ipv4NetworkConfigurator::initialize(int stage) {
    ...
    if (stage == INITSTAGE_LOCAL) {// stage0
        EV_INFO << "!!! stage0 INITSTAGE_LOCAL !!! --> Ipv4NetworkConfigurator::initialize(int stage)\n"; // new added
...
//        EV_INFO <<"!!! assignAddressesParameter: "<<assignAddressesParameter<<"\n"  // new added
//                << "!!! assignUniqueAddresses: "<<assignUniqueAddresses<<"\n"
//                << "!!! assignDisjunctSubnetAddressesParameter: "<<assignDisjunctSubnetAddressesParameter<<"\n"
//                << "!!! addStaticRoutesParameter: "<<addStaticRoutesParameter<<"\n"
//                << "!!! addSubnetRoutesParameter: "<<addSubnetRoutesParameter<<"\n"
//                << "!!! addDefaultRoutesParameter: "<<addDefaultRoutesParameter<<"\n"
//                << "!!! addDirectRoutesParameter: "<<addDirectRoutesParameter<<"\n"
//                <<"!!! optimizeRoutesParameter: "<<optimizeRoutesParameter<<"\n";
    }
    else if (stage == INITSTAGE_NETWORK_CONFIGURATION) {// stage4
        EV_INFO << "!!! stage4 INITSTAGE_NETWORK_CONFIGURATION !!! --> Ipv4NetworkConfigurator::initialize(int stage)\n"; // new added
        ensureConfigurationComputed(topology);
    }
    else if (stage == INITSTAGE_LAST){// stage12
        EV_INFO << "!!! stage12 INITSTAGE_LAST !!! --> Ipv4NetworkConfigurator::initialize(int stage)\n"; // new added
        dumpConfiguration();
    }
}
...
void Ipv4NetworkConfigurator::computeConfiguration() {
    ...
    if (assignAddressesParameter) TIME(assignAddresses(topology));
    ...
    if (addStaticRoutesParameter) {
        ...
        if (autorouteElements.size() == 0) {
            ...
            for (auto & autorouteElement : autorouteElements) TIME(addStaticRoutes(topology, autorouteElement));
        }
    }
    ...
    // !!! read and configure sidshores from the XML configuration
    readManualCidshoreConfiguration(topology); // new added
...
void Ipv4NetworkConfigurator::dumpConfiguration() {
    ...
    if (par("dumpTopology")) TIME(dumpTopology(topology));
    // print links to module output
    if (par("dumpLinks")) TIME(dumpLinks(topology));
    // print unicast and multicast addresses and other interface data to module output
    if (par("dumpAddresses")) TIME(dumpAddresses(topology));
    // print routes to module output
    if (par("dumpRoutes")) TIME(dumpRoutes(topology));
    ...
    if (par("dumpCidshores")) TIME(dumpCidshores(topology)); // new added
}
...
void Ipv4NetworkConfigurator::configureInterface(InterfaceEntry *interfaceEntry) {
    ...
    if (it != topology.interfaceInfos.end()) {
        ...
        if (interfaceInfo->configure) configureInterface(interfaceInfo);
    }
}
...
void Ipv4NetworkConfigurator::configureAllRoutingTables() {
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        Node *node = (Node *)topology.getNode(i);
        if (node->routingTable) configureRoutingTable(node);
    }
}
...
void Ipv4NetworkConfigurator::configureRoutingTable(IIpv4RoutingTable *routingTable) {
    ...
    for (int i = 0; i < NumNodes; i++) {
        ...
        if (node->routingTable == routingTable) {
            EV_INFO << "!!! node->routingTable == routingTable \n";// new added
...
// new added
void Ipv4NetworkConfigurator::configureSidTable(IIpv4SidTable *sidTable) {
...
// new added
void Ipv4NetworkConfigurator::configureCidshoreTable(IIpv4CidshoreTable *cidshoreTable) {
    ...
    for (int i = 0; i < NumNodes; i++) {
        ...
        if (node->cidshoreTable == cidshoreTable) {
            EV_INFO << "    --> node->cidshoreTable == cidshoreTable \n"; // new added
            configureCidshoreTable(node);
        }
    }
}
...
void Ipv4NetworkConfigurator::configureInterface(InterfaceInfo *interfaceInfo) {
    ...
    for (auto & multicastGroup : interfaceInfo->multicastGroups) interfaceData->joinMulticastGroup(multicastGroup);
...
void Ipv4NetworkConfigurator::configureRoutingTable(Node *node) {
    ...
    int SizeStaticRoutes = node->staticRoutes.size();
    EV_INFO<<"    --> SizeStaticRoutes: "<<SizeStaticRoutes<<"\n"; // new added
    for (size_t i = 0; i < SizeStaticRoutes; i++) {
        Ipv4Route *original = node->staticRoutes[i];
        EV_INFO<<"    --> staticRoutes["<< i <<"]: "<<original->getDestination()<<"\n"; // new added
...
// new adde
void Ipv4NetworkConfigurator::configureSidTable(Node *node) {
    EV_INFO<<"!!! --> Ipv4NetworkConfigurator::configureSidTable(Node *node)\n"; // new added
    EV_DETAIL << "Configuring sid table of " << node->getModule()->getFullPath() << ".\n";
    int SizeStaticSids = node->staticSids.size();
    EV_INFO<<"    --> SizeStaticSids: "<<SizeStaticSids<<"\n"; // new added
    for (size_t i = 0; i < SizeStaticSids; i++) {
        Ipv4Sid *original = node->staticSids[i];
        ServiceId serviceId = original->getSid();
        Ipv4Address ipv4Address = original->getIpaddr();
        int port = original->getPort();
        EV_INFO<<"    --> staticSids["<< i <<"]:  "<<" ServiceId= "<<serviceId<<" IpAddr= "<<ipv4Address<<" Port= "<<port<<"\n"; // new added
        Ipv4Sid *clone = new Ipv4Sid();

        clone->setSid(serviceId);
        clone->setIpaddr(ipv4Address);
        clone->setPort(port); // new added
        EV_INFO<<"    --> port: "<<clone->getPort()<<endl;
        clone->setInterface(original->getInterface());
        clone->setSourceType(original->getSourceType());
...
void Ipv4NetworkConfigurator::configureCidTable(Node *node) {
    ...
    for (size_t i = 0; i < SizeStaticCids; i++) {
        ...
        EV_INFO<<"    --> staticCids["<< i <<"]: "<<original->getIpaddr()<<" "<<original->getCid()<<" "<<original->getPort()<<"\n"; // new added

        Ipv4Cid *clone = new Ipv4Cid();
        clone->setSourceType(original->getSourceType());
        clone->setIpaddr(original->getIpaddr());
        clone->setCid(original->getCid());
        clone->setPort(original->getPort()); // new added
        clone->setInterface(original->getInterface());
        node->cidTable->addCid(clone);
    }
}
// new adde
void Ipv4NetworkConfigurator::configureCidshoreTable(Node *node) {
    EV_INFO<<"!!! --> Ipv4NetworkConfigurator::configureCidshoreTable(Node *node)\n"; // new added
    EV_DETAIL << "Configuring cidshore table of " << node->getModule()->getFullPath() << ".\n";
    int SizeStaticCidshores = node->staticCidshores.size();
    EV_INFO<<"    --> SizeStaticCidshores: "<< SizeStaticCidshores <<"\n"; // new added
    for (size_t i = 0; i < SizeStaticCidshores; i++) {
        Ipv4Cidshore *original = node->staticCidshores[i];
        EV_INFO<<"    --> staticCidshores["<< i <<"]: "<< original->getIpaddr() <<" "<< original->getCid() <<" "<< original->getPort() <<" "<< original->getMMSI() <<"\n"; // new added

        Ipv4Cidshore *clone = new Ipv4Cidshore();
        clone->setSourceType(original->getSourceType());
        clone->setIpaddr(original->getIpaddr());
        clone->setCid(original->getCid());
        clone->setPort(original->getPort());
        clone->setMMSI(original->getMMSI()); // new added
        clone->setInterface(original->getInterface());
        node->cidshoreTable->addCidshore(clone);
    }
}
...
void Ipv4NetworkConfigurator::readInterfaceConfiguration(Topology& topology) {
    EV_INFO<<"!!! --> Ipv4NetworkConfigurator::readInterfaceConfiguration(Topology& topology) \n"; // new added
...
// new added
void Ipv4NetworkConfigurator::dumpCidshores(Topology& topology) {
    EV_INFO << "!!! --> Ipv4NetworkConfigurator::dumpCidshores(Topology& topology)\n";// new added
    for (int i = 0; i < topology.getNumNodes(); i++) {
        Node *node = (Node *)topology.getNode(i);
        if (node->cidshoreTable) {
            EV_INFO << "Node " << node->module->getFullPath() << endl;
            check_and_cast<IIpv4CidshoreTable *>(node->cidshoreTable)->printCidshoreTable();
        }
    }
}
...
void Ipv4NetworkConfigurator::dumpConfig(Topology& topology) {
    EV_INFO<<"Ipv4NetworkConfigurator::dumpConfig(Topology& topology)\n"; // new added
    FILE *f;
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        ...
        if (sidTable) {
            for (int j = 0; j < sidTable->getNumSids(); j++) {
                ...
                u_short port = sid->getPort();
                stream << "   <sid hosts=\"" << node->module->getFullPath();

                stream << "\" ServiceId=\"";
                if (serviceId.isUnspecified()) stream << "*";
                else stream << serviceId;

                stream << "\" IpAddress=\"";
                if (ipAddress.isUnspecified()) stream << "*";
                else stream << ipAddress;
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        Node *node = (Node *)topology.getNode(i);
        IIpv4CidshoreTable *cidshoreTable = dynamic_cast<IIpv4CidshoreTable *>(node->cidshoreTable);
        if (cidshoreTable) {
            for (int j = 0; j < cidshoreTable->getNumCidshores(); j++) {
                Ipv4Cidshore *cidshore = cidshoreTable->getCidshore(j);
                std::stringstream stream;
                stream << "   <cidshore hosts=\"" << node->module->getFullPath();

                stream << "\" IpAddress=\"";
                if (cidshore->getIpaddr().isUnspecified()) stream << "*";
                else stream << cidshore->getIpaddr();

                stream << "\" UdpPort=\"";
                stream << cidshore->getPort();

                stream << "\" ClientId=\"";
                if (cidshore->getCid().isUnspecified()) stream << "*";
                else stream << cidshore->getCid();

                stream << "\" MMSI=\"";
                if (cidshore->getMMSI().isUnspecified()) stream << "*";
                else stream << cidshore->getMMSI();
...
void Ipv4NetworkConfigurator::readManualRouteConfiguration(Topology& topology) {
    ...
    for (auto & routeElement : routeElements) {
        ...
        try {
            ...
            if (!isEmpty(destinationAttr) && strcmp(destinationAttr, "*")) destination = resolve(destinationAttr, L3AddressResolver::ADDR_IPv4).toIpv4();

            Ipv4Address netmask;
            if (!isEmpty(netmaskAttr) && strcmp(netmaskAttr, "*")) {
                if (netmaskAttr[0] == '/') netmask = Ipv4Address::makeNetmask(atoi(netmaskAttr + 1));
                else netmask = Ipv4Address(netmaskAttr);
            }
            if (!netmask.isValidNetmask()) throw cRuntimeError("Wrong netmask %s", netmask.str().c_str());
            if (isEmpty(interfaceAttr) && isEmpty(gatewayAttr)) throw cRuntimeError("Incomplete route: either gateway or interface (or both) must be specified");

            // find matching host(s), and add the route
            Matcher atMatcher(hostAttr);
            for (int i = 0; i < topology.getNumNodes(); i++) {
                Node *node = (Node *)topology.getNode(i);
                EV_INFO<<"    --> node->routingTable: "<< node->routingTable << endl; // new added
                if (node->routingTable) {
                    ...
                    if (atMatcher.matches(hostShortenedFullPath.c_str()) || atMatcher.matches(hostFullPath.c_str())) {
                        ...
                        if (isNotEmpty(metricAttr)) route->setMetric(atoi(metricAttr));
...
void Ipv4NetworkConfigurator::readManualSidConfiguration(Topology& topology) {
    ...
    for (auto & sidElement : sidElements) {
        ...
        const char *serviceidAttr = sidElement->getAttribute("sid");
        const char *ipaddrAttr = xmlutils::getMandatoryAttribute(*sidElement, "ipaddr");
        const char *udpportAttr = sidElement->getAttribute("udpport");
        EV_INFO<<"    --> "<<"  serviceid: "<< serviceidAttr <<"  ipaddr: "<< ipaddrAttr <<"  udpport: "<< udpportAttr << endl; //new added
        try {
            // parse and check the attributes
            ServiceId serviceId;
            if (!isEmpty(serviceidAttr) && strcmp(serviceidAttr, "*")) serviceId = resolve(serviceidAttr, L3AddressResolver::ADDR_SERVICEID).toServiceId();
            Ipv4Address ipaddr;
            if (!isEmpty(ipaddrAttr) && strcmp(ipaddrAttr, "*")) ipaddr = resolve(ipaddrAttr, L3AddressResolver::ADDR_IPv4).toIpv4();
            int port;
            if (!isEmpty(udpportAttr) && strcmp(udpportAttr, "*")) port = atoi(udpportAttr);
            EV_INFO<<"    --> port: "<<port<<endl;
            // find matching host(s), and add the sid
            Matcher atMatcher(hostAttr);
            for (int i = 0; i < topology.getNumNodes(); i++) {
                Node *node = (Node *)topology.getNode(i);
                EV_INFO<<"    --> node->sidTable: "<<node->sidTable<<"\n"; // new added
                if (node->sidTable) {
                    std::string hostFullPath = node->module->getFullPath();
                    std::string hostShortenedFullPath = hostFullPath.substr(hostFullPath.find('.') + 1);
                    if (atMatcher.matches(hostShortenedFullPath.c_str()) || atMatcher.matches(hostFullPath.c_str())) {
                        ...
                        ipv4Sid->setIpaddr(ipaddr);
                        ipv4Sid->setPort(port);  // new added
                        ipv4Sid->setInterface(ie);
                        ipv4Sid->setSourceType(ipv4Sid->getSourceType());
                        node->staticSids.push_back(ipv4Sid);
                    }
                }
            }
        }
...
void Ipv4NetworkConfigurator::readManualCidConfiguration(Topology& topology) {
    EV_INFO<<"!!! --> Ipv4NetworkConfigurator::readManualCidConfiguration(Topology& topology) \n";  //new added
    // configuration在NetworkConfiguratorBase::initialize(int stage)定义
    ...
    for (auto & cidElement : cidElements) {
        ...
        const char *udpportAttr = cidElement->getAttribute("udpport");
        EV_INFO<<"    --> ipAddr: "<<ipaddrAttr<<"  clientid: "<<cidAttr<<"  port: "<<udpportAttr<<endl; //new added
        try {
            // parse and check the attributes
            Ipv4Address ipaddr;
            if (!isEmpty(ipaddrAttr) && strcmp(ipaddrAttr, "*")) ipaddr = resolve(ipaddrAttr, L3AddressResolver::ADDR_IPv4).toIpv4();
            ClientId clientId;
            if (!isEmpty(cidAttr) && strcmp(cidAttr, "*")) clientId = resolve(cidAttr, L3AddressResolver::ADDR_CLIENTID).toClientId();
            int port;
            if (!isEmpty(udpportAttr) && strcmp(udpportAttr, "*")) port = atoi(udpportAttr);
            EV_INFO<<"    --> port: "<< port <<endl;
            ...
            for (int i = 0; i < topology.getNumNodes(); i++) {
                ...
                if (node->cidTable) {
                    ...
                    if (atMatcher.matches(hostShortenedFullPath.c_str()) || atMatcher.matches(hostFullPath.c_str())) {
                        ...
                        ipv4Cid->setCid(clientId);
                        ipv4Cid->setPort(port);
                        EV_INFO<<"    --> Port: "<<ipv4Cid->getPort()<<endl;
...
void Ipv4NetworkConfigurator::readManualCidshoreConfiguration(Topology& topology) {
    ...
    for (auto & cidshoreElement : cidshoreElements) {
        ...
        const char *ipaddrAttr = xmlutils::getMandatoryAttribute(*cidshoreElement, "ipaddr");
        const char *cidAttr = cidshoreElement->getAttribute("cid");
        const char *udpportAttr = cidshoreElement->getAttribute("udpport");
        const char *mmsiAttr = cidshoreElement->getAttribute("mmsi");
        EV_INFO<<"    --> ipAddr: "<< ipaddrAttr << "  clientid: " << cidAttr << "  port: "<< udpportAttr <<"  mmsi: "<< mmsiAttr <<endl; //new added
        try {
            // parse and check the attributes
            Ipv4Address ipaddr;
            if (!isEmpty(ipaddrAttr) && strcmp(ipaddrAttr, "*")) ipaddr = resolve(ipaddrAttr, L3AddressResolver::ADDR_IPv4).toIpv4();
            ...
            if (!isEmpty(udpportAttr) && strcmp(udpportAttr, "*")) port = atoi(udpportAttr);
            MacAddress mmsi;
            if (!isEmpty(mmsiAttr) && strcmp(mmsiAttr, "*"))  mmsi = resolve(mmsiAttr, L3AddressResolver::ADDR_MAC).toMac();
            EV_INFO<<"    --> mmsi: " << mmsi << endl;  // new added
            // find matching host(s), and add the cidshore
            Matcher atMatcher(hostAttr);
            for (int i = 0; i < topology.getNumNodes(); i++) {
                Node *node = (Node *)topology.getNode(i);
                EV_INFO<<"    --> node->cidshoreTable: "<< node->cidshoreTable << endl; // new added
                if (node->cidshoreTable) {
                    std::string hostFullPath = node->module->getFullPath();
                    std::string hostShortenedFullPath = hostFullPath.substr(hostFullPath.find('.') + 1);
                    if (atMatcher.matches(hostShortenedFullPath.c_str()) || atMatcher.matches(hostFullPath.c_str())) {
                        ...
                        ipv4Cidshore->setCid(clientId);
                        ipv4Cidshore->setPort(port);
                        ipv4Cidshore->setMMSI(mmsi);
                        EV_INFO<<"    --> mmsi: "<< ipv4Cidshore->getMMSI() <<endl;
                        ipv4Cidshore->setInterface(ie);
                        ipv4Cidshore->setSourceType(ICidshore::MANUAL);
                        node->staticCidshores.push_back(ipv4Cidshore);
                    }
                }
            }
        }
        catch (std::exception& e) {
            throw cRuntimeError("Error in XML <cidshore> element at %s: %s", cidshoreElement->getSourceLocation(), e.what());
        }
    }
}
...
// new added
ICidshoreTable *Ipv4NetworkConfigurator::findCidshoreTable(NetworkConfiguratorBase::Node *node) {
    return L3AddressResolver().findIpv4CidshoreTableOf(node->module);
}
