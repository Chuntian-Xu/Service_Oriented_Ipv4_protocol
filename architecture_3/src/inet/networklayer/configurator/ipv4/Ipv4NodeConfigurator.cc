...
void Ipv4NodeConfigurator::initialize(int stage) {
    ...
    if (stage == INITSTAGE_LOCAL) {  // stage0
        ...
        cidTable = getModuleFromPar<IIpv4CidTable>(par("cidTableModule"), this);  // !!! get cidTableModule
        EV_INFO<< "    --> get cidTable: " << cidTable->getNumCids() <<"\n";  // new added
        ...
        if ((!nodeStatus || nodeStatus->getState() == NodeStatus::UP) && networkConfigurator) {
            ...
            configureCidshoreTable(); // new added
...
// new added ^-^
void Ipv4NodeConfigurator::configureCidshoreTable() {
    EV_INFO<<"\n !!! --> Ipv4NodeConfigurator::configureCidshoreTable()\n"; // new added
    ASSERT(networkConfigurator);
    if (par("configureCidshoreTable")) networkConfigurator->configureCidshoreTable(cidshoreTable);
}
