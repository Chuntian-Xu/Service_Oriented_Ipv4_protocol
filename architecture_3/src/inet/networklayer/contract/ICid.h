...
#ifndef __INET_ICID_H
#define __INET_ICID_H
...
/**
 * C++ interface for accessing ClientId table entries of various protocols (IPv4, IPv6, etc) in a uniform way.
 *
 * @see ICidTable
 */
class INET_API ICid {
  public:
    /** Specifies where the cid comes from */
    enum SourceType {
    ...
    virtual ICidTable *getCidTableAsGeneric() const = 0;

    /** operation: set */
    virtual void setIpaddr(const L3Address& ipaddr) = 0;
    virtual void setCid(const L3Address& cid) = 0;
    virtual void setPort(int port) = 0; // new added
    ...
    virtual ClientId getCidAsGeneric() const = 0;
    virtual ClientId getCid() const = 0;
    virtual int getPort() const = 0; // new added
...
inline std::ostream& operator<<(std::ostream& out, const ICid *cid) {
    out << "ipaddr = " << cid->getIpaddrAsGeneric();
    out << "port = " << cid->getPort();
    out << "client id = " << cid->getCidAsGeneric();
    if (auto ie = cid->getInterface()) out << ", interface = " << ie->getInterfaceName();
    return out;
};

} // namespace inet

#endif // ifndef __INET_ICID_H
