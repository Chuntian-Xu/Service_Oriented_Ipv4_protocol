...
Define_Module(Ipv4_MobileStation);

Ipv4_MobileStation::Ipv4_MobileStation()
{
}
...
// ^-^
void Ipv4_MobileStation::initialize(int stage) {
...
void Ipv4_MobileStation::handleRegisterService(const Protocol& protocol, cGate *out, ServicePrimitive servicePrimitive)
{
    Enter_Method("handleRegisterService");
}

void Ipv4_MobileStation::handleRegisterProtocol(const Protocol& protocol, cGate *in, ServicePrimitive servicePrimitive)
{
...
void Ipv4_MobileStation::refreshDisplay() const
{
...
// ^-^
void Ipv4_MobileStation::handleMessageWhenUp(cMessage *msg) {
    ...
    if (msg->arrivedOn("transportIn")) {    //TODO packet->getArrivalGate()->getBaseId() == transportInGateBaseId
        ...
        if(packet->hasAtFront<Ipv4Header>()) {
            handleIncomingDatagram(packet);  // ^-^ new added !!!
        }
        else if(packet->hasAtFront<Ipv4ServiceHeader>()) {
            handleIncomingDatagram_Service(packet);  // ^-^ new added !!!
        }
    }
    else throw cRuntimeError("message arrived on unknown gate '%s'", msg->getArrivalGate()->getName());
}

bool Ipv4_MobileStation::verifyCrc(const Ptr<const Ipv4Header>& ipv4Header)
{
...
const InterfaceEntry *Ipv4_MobileStation::getSourceInterface(Packet *packet)
{
...
// ^-^ new added !!! Remove and parse the Ipv4serviceHeader, reconstruct the UdpHeader and Ipv4Header, then route to dest host
void Ipv4_MobileStation::handleIncomingDatagram_Service(Packet *packet) {
    ...
    clientId_c = clientId;
    EV_INFO<<"    --> clientId_c: "<< clientId_c << endl;
    //----------------- Inquiry the sid_table of MobileStation for srcIpaddr and srcPort -----------------
    ...
    for(int i=0; i<sidNum; ++i) {
        ...
        if(sid->getSid() == serviceId) {
            srcIpaddr = sid->getIpaddr();
            srcPort = sid->getPort();
            EV_INFO<<"    --> serviceId: "<<sid->getSid()<<"  srcIpaddr: "<< srcIpaddr <<"  srcPort: "<< srcPort << endl;
        }
    }
    for(int i=0; i<cidNum; ++i) {
        Ipv4Cid* cid = ct->getCid(i);
        if(cid->getCid() == clientId) {
            destIpaddr = cid->getIpaddr();
            destPort = cid->getPort();
            EV_INFO<<"    --> clientId: "<< cid->getCid() << "  destIpaddr: " << destIpaddr << "  destPort: " << destPort << endl;
        }
    }
    if(destPort == 0){
        EV_INFO<<"    --> Send broadcast packet ! "<<endl;
        Ipv4Address ipaddr_MobileStation = ift->findInterfaceByName("eth0")->getIpv4Address();
        Ipv4Address netmask_MobileStation = ift->findInterfaceByName("eth0")->getIpv4Netmask();
        EV_INFO<<"    --> ipaddr_MobileStation: "<< ipaddr_MobileStation << endl <<"    --> netmask_MobileStation: "<< netmask_MobileStation << endl;
        destIpaddr = ipaddr_MobileStation.makeBroadcastAddress(netmask_MobileStation);
        destPort = srcPort;  // Port dedicated to receiving broadcasts
        EV_INFO<<"    --> destIpaddr: " << destIpaddr << "  destPort: " << destPort << endl;
    }
...
// ^-^
Packet *Ipv4_MobileStation::prepareForForwarding(Packet *packet) const {
    EV_INFO<<"!!! --> Ipv4_MobileStation::prepareForForwarding(Packet *packet) \n"; // new added
...
// Determine the interface type and destination IP address type: isMulticast? isLocalAddress? isLimitedBroadcastAddress?
void Ipv4_MobileStation::preroutingFinish(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::preroutingFinish(Packet *packet)\n"; // new added
    EV_INFO<<"    --> packet: "<<packet<<"\n"; // new added
    ...
    EV_INFO<<"    --> nextHopAddr: "<<nextHopAddr<<"\n"; // new added
...
//            EV_INFO<<"    --> isLocalAddress\n";// new added
            ...
            EV_INFO << "    --> packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(nextHopAddr)"<<endl; // new added
...
// ^-^
void Ipv4_MobileStation::handlePacketFromHL(Packet *packet) {
    EV_INFO <<"!!! --> Ipv4_MobileStation::handlePacketFromHL(Packet *packet)" << "\n"; // new added
...
// ^-^
void Ipv4_MobileStation::datagramLocalOut(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::datagramLocalOut(Packet *packet)\n"; // new added
 ...
 */
const InterfaceEntry *Ipv4_MobileStation::determineOutgoingInterfaceForMulticastDatagram(const Ptr<const Ipv4Header>& ipv4Header, const InterfaceEntry *multicastIFOption)
{
 ...
 */
void Ipv4_MobileStation::routeUnicastPacket(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::routeUnicastPacket(Packet *packet)\n"; // new added
    EV_INFO<<"    --> packet: "<<packet<<"\n"; // new added
    const InterfaceEntry *fromIE = getSourceInterface(packet);
    const InterfaceEntry *destIE = getDestInterface(packet);
    Ipv4Address nextHopAddress = getNextHop(packet);
    EV_INFO<<"    --> nextHopAddress: "<<nextHopAddress<<"\n"; // new added
...
//        EV_INFO<<"    !!--> nextHopAddress: "<<getNextHop(packet)<<"\n"; // new added
...
//        EV_INFO << "   --> Destination: " <<re->getDestination()<<endl;  // new added
//        EV_INFO << "   --> Netmask: " <<re->getNetmask()<<endl;  // new added
//        EV_INFO << "   --> Gateway: " <<re->getGateway()<<endl;  // new added
//        EV_INFO << "   --> Iface: " <<destIE->getInterfaceName()<<endl;  // new added
        EV_INFO << "   --> packet->addTagIfAbsent<InterfaceReq>()->setInterfaceId(destIE->getInterfaceId())" <<endl;  // new added
        EV_INFO << "   --> packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(re->getGateway())" <<endl;  // new added
    }
...
void Ipv4_MobileStation::routeLocalBroadcastPacket(Packet *packet)
{
    EV_INFO<<"!!! --> Ipv4_MobileStation::routeLocalBroadcastPacket(Packet *packet)"<<endl;  // new added
...
const InterfaceEntry *Ipv4_MobileStation::getShortestPathInterfaceToSource(const Ptr<const Ipv4Header>& ipv4Header) const
{
    return rt->getInterfaceForDestAddr(ipv4Header->getSrcAddress());
}

void Ipv4_MobileStation::forwardMulticastPacket(Packet *packet)
{
    const InterfaceEntry *fromIE = ift->getInterfaceById(packet->getTag<InterfaceInd>()->getInterfaceId());
    const auto& ipv4Header = packet->peekAtFront<Ipv4Header>();
...
// ^-^
void Ipv4_MobileStation::reassembleAndDeliver(Packet *packet) {
    EV_INFO << "!!! --> Ipv4_MobileStation::reassembleAndDeliver(Packet *packet) \n"; // new added
    EV_INFO << "Delivering " << packet << " locally.\n";
...
// ^-^
void Ipv4_MobileStation::reassembleAndDeliverFinish(Packet *packet) {
    EV_INFO << "!!! --> Ipv4_MobileStation::reassembleAndDeliverFinish(Packet *packet) \n"; // new added
...
// ^-^  !!!
void Ipv4_MobileStation::decapsulate(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::decapsulate(Packet *packet) \n";  // new added
 ...
 */
void Ipv4_MobileStation::fragmentPostRouting(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::fragmentPostRouting(Packet *packet) \n"; // new added
//    EV_INFO<<"    --> packet: "<<packet<<"\n"; // new added
...
void Ipv4_MobileStation::setComputedCrc(Ptr<Ipv4Header>& ipv4Header)
{
...
void Ipv4_MobileStation::insertCrc(const Ptr<Ipv4Header>& ipv4Header)
{
 ...
 */
void Ipv4_MobileStation::fragmentAndSend(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::fragmentAndSend(Packet *packet) \n";  // new added
//    EV_INFO<<"    --> packet: "<<packet<<"\n"; // new added
    const InterfaceEntry *destIE = ift->getInterfaceById(packet->getTag<InterfaceReq>()->getInterfaceId());
    Ipv4Address nextHopAddr = getNextHop(packet);
...
// ^-^   !!!
void Ipv4_MobileStation::encapsulate(Packet *transportPacket) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::encapsulate(Packet *transportPacket) \n"; // new added
    const auto& ipv4Header = makeShared<Ipv4Header>();
...
// ^-^
void Ipv4_MobileStation::sendDatagramToOutput(Packet *packet) {
    EV_INFO << "!!! --> Ipv4_MobileStation::sendDatagramToOutput(Packet *packet) \n"; // new added
    EV_INFO<<"    --> packet: "<<packet<<"\n";// new added
...
// ^-^  new added !!! Remove and parse the Ipv4Header and UdpHeader, add the Ipv4ServiceHeader, and then send it down to the Mac layer through the network port
void Ipv4_MobileStation::sendDatagramToOutput_Service(Packet *packet) {
    ...
    for(int i=0; i<cidNum; ++i) {
        Ipv4Cid* cid = ct->getCid(i);
        if((cid->getIpaddr() == srcIpaddr) && (cid->getPort() == srcPort)) {
            clientId = cid->getCid();
            EV_INFO<<"    --> srcIpaddr: "<<cid->getIpaddr()<<"  srcPort: "<< srcPort <<"  clientId: "<<clientId<< endl;
            clientId_c = clientId;
            EV_INFO<<"    --> clientId_c: "<< clientId_c << endl;
        }
    }
    for(int i=0; i<sidNum; ++i) {
        Ipv4Sid* sid = st->getSid(i);
        if((sid->getIpaddr() == destIpaddr) && (sid->getPort() == destPort) ) {
            serviceId = sid->getSid();
            EV_INFO<<"    --> destIpaddr: "<<sid->getIpaddr()<<"  destPort: "<< destPort <<"  serviceId: "<<serviceId<< endl;
        }
    }
    if(clientId.isUnspecified() || serviceId.isUnspecified()) {
        clientId = clientId_c;
        EV_INFO<<"    --> clientId: "<< clientId << endl;
    }

    //----------------- construct the Ipv4_Service packet -----------------
    ...
    MacAddress nextHopMacAddr = MacAddress("FF-FF-FF-FF-FF-FF");
...
void Ipv4_MobileStation::arpResolutionCompleted(IArp::Notification *entry)
{
...
void Ipv4_MobileStation::arpResolutionTimedOut(IArp::Notification *entry)
{
    if (entry->l3Address.getType() != L3Address::IPv4)
        return;
    auto it = pendingPackets.find(entry->l3Address.toIpv4());
...
MacAddress Ipv4_MobileStation::resolveNextHopMacAddress(cPacket *packet, Ipv4Address nextHopAddr, const InterfaceEntry *destIE)
{
    EV_INFO<<"!!! --> Ipv4_MobileStation::resolveNextHopMacAddress(cPacket *packet, Ipv4Address nextHopAddr, const InterfaceEntry *destIE)"<<endl; // new added
...
// ^-^
void Ipv4_MobileStation::sendPacketToNIC(Packet *packet) {
...
// NetFilter:
void Ipv4_MobileStation::registerHook(int priority, INetfilter::IHook *hook)
{
    Enter_Method("registerHook()");
    NetfilterBase::registerHook(priority, hook);
}

void Ipv4_MobileStation::unregisterHook(INetfilter::IHook *hook)
{
    Enter_Method("unregisterHook()");
    NetfilterBase::unregisterHook(hook);
}

void Ipv4_MobileStation::dropQueuedDatagram(const Packet *packet)
{
...
void Ipv4_MobileStation::reinjectQueuedDatagram(const Packet *packet)
{
...
// ^-^
INetfilter::IHook::Result Ipv4_MobileStation::datagramPreRoutingHook(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::datagramPreRoutingHook(Packet *packet) \n"; // new added
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result Ipv4_MobileStation::datagramForwardHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
// ^-^
INetfilter::IHook::Result Ipv4_MobileStation::datagramPostRoutingHook(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_MobileStation::datagramPostRoutingHook(Packet *packet)\n";  //new added
    for (auto & elem : hooks) {
...
void Ipv4_MobileStation::handleStartOperation(LifecycleOperation *operation)
{
    start();
}

void Ipv4_MobileStation::handleStopOperation(LifecycleOperation *operation)
{
    // TODO: stop should send and wait pending packets
    stop();
}

void Ipv4_MobileStation::handleCrashOperation(LifecycleOperation *operation)
{
    stop();
}

void Ipv4_MobileStation::start()
{
}

void Ipv4_MobileStation::stop()
{
    flush();
...
INetfilter::IHook::Result Ipv4_MobileStation::datagramLocalInHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
// ^-^
INetfilter::IHook::Result Ipv4_MobileStation::datagramLocalOutHook(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4::datagramLocalOutHook(Packet *packet) \n";
    for (auto & elem : hooks) {
...
void Ipv4_MobileStation::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    Enter_Method_Silent();
...
void Ipv4_MobileStation::sendIcmpError(Packet *origPacket, int inputInterfaceId, IcmpType type, IcmpCode code)
{
