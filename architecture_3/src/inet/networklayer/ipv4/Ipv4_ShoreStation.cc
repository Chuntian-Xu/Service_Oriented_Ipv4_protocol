...
void Ipv4_ShoreStation::handleIncomingDatagram(Packet *packet) {
    ...
    // "Prerouting"
    const auto& ipv4Header = packet->peekAtFront<Ipv4Header>();  // !!!--> ??? ipv4Header
...
void Ipv4_ShoreStation::handleIncomingDatagram_Service(Packet *packet) {
    ...
    for(int i=0; i<cidshoreNum; ++i) {
        ...
        if((cidsh->getCid() == clientId) && (cidsh->getMMSI() == macAddressInd->getSrcAddress()) )  {
            srcIpaddr = cidsh->getIpaddr();
            srcPort = cidsh->getPort();
            EV_INFO<<"    --> MMSI: "<< cidsh->getMMSI() <<"  clientId: "<< cidsh->getCid() << "  srcIpaddr: "<< srcIpaddr << "  srcPort: "<< srcPort << endl;
        }
    }
...
void Ipv4_ShoreStation::sendDatagramToOutput_Service(Packet *packet) {
    ...
    for(int i=0; i<cidshoreNum; ++i) {
        ...
        if((cidsh->getIpaddr() == destIpaddr) && (cidsh->getPort() == destPort)) {
            clientId = cidsh->getCid();
            MMSI = cidsh->getMMSI();
            EV_INFO<<"    --> destIpaddr: "<<cidsh->getIpaddr()<<"  destPort: "<< destPort <<"  MMSI: "<< MMSI <<"  clientId: "<<clientId<< endl;
        }
    }
