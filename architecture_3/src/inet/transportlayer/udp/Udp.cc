...
{
    EV_INFO<<"!!! --> Udp::initialize(int stage)"<<endl; // new added
...
{
    EV_INFO<<"!!! --> Udp::handleLowerPacket(Packet *packet)"<<endl; // new added
    // received from IP layer
    ASSERT(packet->getControlInfo() == nullptr);
    auto protocol = packet->getTag<PacketProtocolTag>()->getProtocol();
//    EV<<"    --> protocol: "<<protocol->str()<<endl;  // new added
    ...
    else if (protocol == &Protocol::ipv4_service) {  // new added
        processPacketFromIpLayer(packet); // new added
    } // new added
    else
        throw cRuntimeError("Unknown protocol: %s(%d)", protocol->getName(), protocol->getId());
}

void Udp::processPacketFromIpLayer(Packet *packet)
{
    ...
    EV_INFO<<"    --> Passing down to Ip Layer" << endl; // new added
    send(packet, "ipOut");
}

void Udp::handleUpperCommand(cMessage *msg)
{
    EV_INFO<<"!!! --> Udp::handleUpperCommand(cMessage *msg)"<<endl; // new added
    switch (msg->getKind()) {
        case UDP_C_BIND: {
...
{
    EV_INFO<<"!!! --> Udp::bind(int sockId, int gateIndex, const L3Address& localAddr, int localPort)"<<endl; // new added
    if (sockId == -1)
...
{
    EV_INFO<<"!!! --> *Udp::findFirstSocketByLocalAddress(const L3Address& localAddr, ushort localPort)"<<endl; // new added
...
{
    EV_INFO<<"!!! --> *Udp::createSocket(int sockId, const L3Address& localAddr, int localPort)"<<endl; // new added
...
{
    EV_INFO<<"!!! --> Udp::getEphemeralPort()"<<endl; // new added
...
{
    EV_INFO<<"!!! --> Udp::connect(int sockId, int gateIndex, const L3Address& remoteAddr, int remotePort)"<<endl; // new added
...
{
    EV_INFO<<"!!! --> *Udp::getOrCreateSocket(int sockId)"<<endl; // new added
    // validate sockId
    if (sockId == -1)
...
{
    EV_INFO<<"!!! --> Udp::setTimeToLive(SockDesc *sd, int ttl)"<<endl; // new added
    sd->ttl = ttl;
}

void Udp::setDscp(SockDesc *sd, short dscp)
{
    EV_INFO<<"!!! --> Udp::setDscp(SockDesc *sd, short dscp)"<<endl; // new added
    sd->dscp = dscp;
}

void Udp::setTos(SockDesc *sd, short tos)
{
    EV_INFO<<"!!! --> Udp::setTos(SockDesc *sd, short tos)"<<endl; // new added
    sd->tos = tos;
}

void Udp::setBroadcast(SockDesc *sd, bool broadcast)
{
    EV_INFO<<"!!! --> Udp::setBroadcast(SockDesc *sd, bool broadcast)"<<endl; // new added
    sd->isBroadcast = broadcast;
}

void Udp::setMulticastOutputInterface(SockDesc *sd, int interfaceId)
{
    EV_INFO<<"!!! --> Udp::setMulticastOutputInterface(SockDesc *sd, int interfaceId)"<<endl; // new added
...
{
    EV_INFO<<"!!! --> Udp::handleUpperPacket(Packet *packet)"<<endl; // new added
...
{
    EV_INFO<<"!!! --> Udp::processUDPPacket(Packet *udpPacket)"<<endl;  // new added
    ...
    if (!isMulticast && !isBroadcast) {
        ...
        if (!sd) {
            EV_WARN << "No socket registered on port(isUnicast) " << destPort << "\n";
        ...
        EV_INFO<<"    --> isMulticastBroadcastAddress "<<endl;  // new added
        ...
        if (sds.empty()) {
            EV_WARN << "No socket registered on port(isLimitedBroadcastAddress) " << destPort << "\n";
            udpPacket->setFrontOffset(udpHeaderPopPosition);
            processUndeliverablePacket(udpPacket);
            return;
        }
...
Udp::SockDesc *Udp::findSocketForUnicastPacket(const L3Address& localAddr, ushort localPort, const L3Address& remoteAddr, ushort remotePort)
{
    ...
    // first check if exists in bound to localPort，discard packet if none found
    auto it = socketsByPortMap.find(localPort);
    if (it == socketsByPortMap.end()){
    ...
    for (SockDescList::reverse_iterator it = list.rbegin(); it != list.rend(); ++it) {
                ...
                (sd->remoteAddr.isUnspecified() || sd->remoteAddr == remoteAddr)))
        {
            if (sd->localAddr.isUnspecified()){
                EV_INFO << "    --> socketBoundToAnyAddress = sd;" << endl;
                socketBoundToAnyAddress = sd;
            }
            else{
                EV_INFO << "    --> return sd;" << endl;
                return sd;
            }
        }
    }
    EV_INFO << "    --> return socketBoundToAnyAddress;" << endl;
