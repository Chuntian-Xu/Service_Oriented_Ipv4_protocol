...
{
    EV_INFO<<"!!! --> GlobalArp::resolveL3Address(const L3Address& address, const InterfaceEntry *interfaceEntry)"<<endl;  // new added
    Enter_Method_Silent();
    if (address.isUnicast()) {
        MacAddress macAddress = mapUnicastAddress(address);
        EV_INFO<<"    --> get macAddress: "<< macAddress << endl; // new added
        return macAddress;
    }
    else if (address.isMulticast()) return mapMulticastAddress(address);
    else if (address.isBroadcast()) return MacAddress::BROADCAST_ADDRESS;
...
{
    EV_INFO<<"!!! --> GlobalArp::mapUnicastAddress(L3Address address)"<<endl;  // new added
