...
{
    EV_INFO<<"!!! --> Ipv4::handleIncomingDatagram(Packet *packet)"<<endl; // new added
    ASSERT(packet);
...
{
    EV_INFO<<"!!! --> Ipv4::prepareForForwarding(Packet *packet) const"<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::preroutingFinish(Packet *packet)"<<endl;  // new added
    ...
    EV_INFO<<"    --> DestAddress: "<< destAddr <<endl;  // new added
        ...
        if (rt->isLocalAddress(destAddr) || fromIE->getProtocolData<Ipv4InterfaceData>()->getIPAddress().isUnspecified()) {
            ...
            if (broadcastIE && fromIE != broadcastIE && rt->isForwardingEnabled()) {
                    ...
                    directBroadcastInterfaceMatcher.matches(broadcastIE->getInterfaceFullPath().c_str()))
                {
                    ...
                    EV_INFO<<"    --> nextHopAddr:"<<getNextHop(packetCopy)<<endl;  // new added
...
{
    EV_INFO << "!!! --> Ipv4::handlePacketFromHL(Packet *packet) "<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::datagramLocalOut(Packet *packet)"<<endl;  // new added
    ...
    if (ipv4Header->getDestAddress().isMulticast()) {
        ...
        // isunicast
            ...
            EV_INFO<<"    --> NextHopAddress: "<<requestedNextHopAddress<<endl;  // new added
            packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(requestedNextHopAddress);
            EV_INFO<<"    --> packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(requestedNextHopAddress) "<<endl;  // new added
            routeUnicastPacket(packet);
        }
    }
}
...
/**
 * Performs unicast routing. Based on the routing decision, it sends the datagram through the outgoing interface.
 */
...
{
    EV_INFO<<"!!! --> Ipv4::routeUnicastPacketFinish(Packet *packet)"<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::routeLocalBroadcastPacket(Packet *packet)"<<endl;  // new added
    ...
    if (destIE != nullptr) {
        ...
        EV_INFO<<"   --> destAddr: "<<destAddr<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::reassembleAndDeliver(Packet *packet)"<<endl;  // new added
    EV_INFO << "Delivering " << packet << " locally.\n";
...
{
    EV_INFO<<"!!! --> Ipv4::reassembleAndDeliverFinish(Packet *packet)"<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::fragmentPostRouting(Packet *packet)"<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::fragmentAndSend(Packet *packet)"<<endl;  // new added
    const InterfaceEntry *destIE = ift->getInterfaceById(packet->getTag<InterfaceReq>()->getInterfaceId());
    Ipv4Address nextHopAddr = getNextHop(packet);
    EV_INFO<<"    --> nextHopAddr:"<<nextHopAddr<<endl;  // new added
    if (nextHopAddr.isUnspecified()) {
        ...
        packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(nextHopAddr);
        EV_INFO<<"    --> nextHopAddr = Dest Ipv4Header:"<<nextHopAddr<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::encapsulate(Packet *transportPacket)"<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::sendDatagramToOutput(Packet *packet)"<<endl;  // new added
...
{
    EV_INFO<<"!!! --> Ipv4::resolveNextHopMacAddress(cPacket *packet, Ipv4Address nextHopAddr, const InterfaceEntry *destIE)"<<endl; // new added
    ...
    MacAddress macAddress = arp->resolveL3Address(nextHopAddr, destIE);
    return macAddress;
}

void Ipv4::sendPacketToNIC(Packet *packet)
{
    EV_INFO << "!!! --> Ipv4::sendPacketToNIC(Packet *packet) " <<endl;  // new added
    ...
    EV_INFO << "    --> packet->addTagIfAbsent<PacketProtocolTag>()->setProtocol(&Protocol::ipv4)"<<endl; // new added
    EV_INFO << "    --> packet->addTagIfAbsent<DispatchProtocolInd>()->setProtocol(&Protocol::ipv4)"<<endl; // new added
