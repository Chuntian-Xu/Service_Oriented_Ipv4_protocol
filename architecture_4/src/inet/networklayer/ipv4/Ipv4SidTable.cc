...
std::ostream& operator<<(std::ostream& os, const Ipv4Sid& ipv4Sid) {
    os << ipv4Sid.str();
    return os;
};
...
bool Ipv4SidTable::sidLessThan(const Ipv4Sid *a, const Ipv4Sid *b) const {
    ...
    // for the same ip address: smaller ServiceId is better (if useAdminDist)
...
//    EV_INFO << "!!! --> Ipv4SidTable::internalAddSid(Ipv4Sid *entry) \n"; // new added
    ...
    invalidateCache();
    ipv4Sid->setSidTable(this);
}

void Ipv4SidTable::addSid(Ipv4Sid *ipv4Sid) {
//    EV_INFO << "!!! --> Ipv4SidTable::addSid(Ipv4Sid *entry) \n"; // new added
    Enter_Method("addSid(...)");
    // This method should be called before calling entry->str()
    internalAddSid(ipv4Sid);
    EV_INFO << "add sid " << ipv4Sid->str() << "\n";
    emit(sidAddedSignal, ipv4Sid);
}

Ipv4Sid *Ipv4SidTable::internalRemoveSid(Ipv4Sid *ipv4Sid) {
EV_INFO<<"!!! --> *Ipv4SidTable::internalRemoveSid(Ipv4Sid *entry)"<<endl;  // new added
    auto i = std::find(sids.begin(), sids.end(), ipv4Sid);
    if (i != sids.end()) {
        sids.erase(i);
        invalidateCache();
        return ipv4Sid;
...
void Ipv4SidTable::ipv4SidChanged(Ipv4Sid *ipv4Sid, int fieldCode) {
    EV_INFO<<"!!! --> Ipv4SidTable::ipv4SidChanged(Ipv4Sid *entry, int fieldCode)"<<endl;  // new added
    if (fieldCode == Ipv4Sid::F_DESTINATION || fieldCode == Ipv4Sid::F_PREFIX_LENGTH || fieldCode == Ipv4Sid::F_METRIC|| fieldCode == Ipv4Sid::F_PORT) {    // our data structures depend on these fields
        ipv4Sid = internalRemoveSid(ipv4Sid);
        ASSERT(ipv4Sid != nullptr);    // failure means inconsistency: route was not found in this routing table
        internalAddSid(ipv4Sid);
    }
    emit(sidChangedSignal, ipv4Sid);    // TODO include fieldCode in the notification
}
...
bool Ipv4SidTable::handleOperationStage(LifecycleOperation *operation, IDoneCallback *doneCallback) {
    ...
    if (dynamic_cast<ModuleStartOperation *>(operation)) {
        if (static_cast<ModuleStartOperation::Stage>(stage) == ModuleStartOperation::STAGE_NETWORK_LAYER) {
            ...
            const char *filename = par("sidFile");
