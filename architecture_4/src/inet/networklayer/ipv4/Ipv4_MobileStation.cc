...
void Ipv4_MobileStation::handleIncomingDatagram_Service(Packet *packet) {
    ...
    destPort = srcPort;
    EV_INFO<<"    --> destIpaddr: "<< destIpaddr << endl <<"    --> destPort: "<< destPort << endl;
    ...
    if (ipv4Header->getDestAddress().isMulticast()) {
        ...
        if (destIE) {
            ...
            delete packet;
              ...
              // check for local delivery
        EV_INFO<<"    --> Unicast or Broadcast"<<endl;
        if (rt->isLocalAddress(destAddr)) {
            ...
            EV_INFO<<"    --> Broadcast"<<endl;
            routeLocalBroadcastPacket(packet);
        }

        else {
            EV_INFO<<"    --> Unicast"<<endl;
...
void Ipv4_MobileStation::routeLocalBroadcastPacket(Packet *packet)
{
    ...
    if (destIE != nullptr) {
        ...
        EV_INFO<<"    --> limitedBroadcast"<<endl;  // new added
...
//    EV_INFO<<"    --> packet: "<<packet<<"\n";// new added
    ...
    EV_INFO<<"    --> nextHopAddr: " << nextHopAddr << endl;// new added
    ...
    if (!ie->isBroadcast() || ie->getMacAddress().isUnspecified()) // we can't do ARP
        ...
        EV_INFO<<"    --> nextHopMacAddr: " << nextHopMacAddr << endl;// new added
...
void Ipv4_MobileStation::sendDatagramToOutput_Service(Packet *packet) {
    ...
    if(serviceId.isUnspecified()) {
        EV_ERROR<<"    --> no such service, drop this packet !\n"<<endl;
        return;
    }
