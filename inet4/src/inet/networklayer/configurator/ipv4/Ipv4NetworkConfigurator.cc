...
//        EV_INFO<<"!!! staticSids["<< i <<"]: "<<original->getIpaddr()<<" "<<original->getSid()<<"\n"; // new added
...
//    EV_INFO<<"!!! --> Ipv4NetworkConfigurator::readManualRouteConfiguration(Topology& topology) \n"; // new added
    ...
    for (auto & routeElement : routeElements) {
        ...
        try {
            ...
            for (int i = 0; i < topology.getNumNodes(); i++) {
                Node *node = (Node *)topology.getNode(i);
//                EV_INFO<<"!!!!!!!!! node->routingTable: "<<node->routingTable<<"\n"; // new added
                if (node->routingTable) {
...
//    EV_INFO<<"\n !!! --> Ipv4NetworkConfigurator::readSidConfiguration(Topology& topology) \n";
    // configuration在NetworkConfiguratorBase::initialize(int stage)定义
    ...
    for (auto & sidElement : sidElements) {
        ...
        const char *sidAttr = sidElement->getAttribute("sid");
//        EV_INFO<<"!!! ipaddr: "<<ipaddrAttr<<"  "<<"sid: "<<sidAttr<<"\n"; //new added
        try {
            // parse and check the attributes
            Ipv4Address ipaddr;
            if (!isEmpty(ipaddrAttr) && strcmp(ipaddrAttr, "*")) ipaddr = resolve(ipaddrAttr, L3AddressResolver::ADDR_IPv4).toIpv4();
            ServiceId sid;
            if (!isEmpty(sidAttr) && strcmp(sidAttr, "*")) sid = resolve(sidAttr, L3AddressResolver::ADDR_SERVICEID).toServiceId();
...
//                EV_INFO<<"!!!!!!!!! node->sidTable: "<<node->sidTable<<"\n"; // new added
                if (node->sidTable) {
                    ...
                    if (atMatcher.matches(hostShortenedFullPath.c_str()) || atMatcher.matches(hostFullPath.c_str())) {
                        ...
                        ipv4Sid->setSourceType(ISid::MANUAL);
