...
#include "headers/service_client_id.h"
...
const Ptr<Chunk> Ipv4ServiceHeaderSerializer::deserialize(MemoryInputStream& stream) const
{
    ...
    const struct service_client_id& id = *static_cast<const struct service_client_id *>((void *)&buffer);

    ipv4ServiceHeader->setClientId(ClientId(uint8(ntohl(id.client_id.s_addr))));
