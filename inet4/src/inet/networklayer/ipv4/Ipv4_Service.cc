...
#include "inet/networklayer/ipv4/Ipv4_Service.h"
namespace inet {

Define_Module(Ipv4_Service);

Ipv4_Service::Ipv4_Service()
{
}
...
// ^-^
void Ipv4_Service::initialize(int stage) {
...
void Ipv4_Service::handleRegisterService(const Protocol& protocol, cGate *out, ServicePrimitive servicePrimitive)
{
    Enter_Method("handleRegisterService");
}

void Ipv4_Service::handleRegisterProtocol(const Protocol& protocol, cGate *in, ServicePrimitive servicePrimitive)
{
...
void Ipv4_Service::refreshDisplay() const
{
...
void Ipv4_Service::handleRequest(Request *request) {
    EV_INFO<<"Ipv4_Service::handleRequest(Request *request) \n"; // new added
...
// ^-^
void Ipv4_Service::handleMessageWhenUp(cMessage *msg) {
    EV_INFO<<"!!! --> Ipv4_Service::handleMessageWhenUp(cMessage *msg)\n"; // new added
...
bool Ipv4_Service::verifyCrc(const Ptr<const Ipv4Header>& ipv4Header)
{
...
const InterfaceEntry *Ipv4_Service::getSourceInterface(Packet *packet)
{
...
// ^-^
const InterfaceEntry *Ipv4_Service::getDestInterface(Packet *packet) {
    EV_INFO<<"\n    --> Ipv4_Service::getDestInterface(Packet *packet)\n"; // new added
    auto tag = packet->findTag<InterfaceReq>();
    return tag != nullptr ? ift->getInterfaceById(tag->getInterfaceId()) : nullptr;
}

// ^-^
Ipv4Address Ipv4_Service::getNextHop(Packet *packet) {
    EV_INFO<<"\n    --> Ipv4_Service::getNextHop(Packet *packet)\n";
    auto tag = packet->findTag<NextHopAddressReq>();
    return tag != nullptr ? tag->getNextHopAddress().toIpv4() : Ipv4Address::UNSPECIFIED_ADDRESS;
}

// ^-^
void Ipv4_Service::handleIncomingDatagram(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_Service::handleIncomingDatagram(Packet *packet) \n"; // new added
    ASSERT(packet);
    ...
    if (ipv4Header->getTotalLengthField() < packet->getDataLength()) {
        EV_INFO<<"!!!!!!--> Fragmented"; // new added 未调用
...
// ^-^
Packet *Ipv4_Service::prepareForForwarding(Packet *packet) const {
    EV_INFO<<"    --> Ipv4_Service::prepareForForwarding(Packet *packet) \n"; // new added
...
// ^-^
void Ipv4_Service::preroutingFinish(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_Service::preroutingFinish(Packet *packet)\n"; // new added
...
// ^-^
void Ipv4_Service::handlePacketFromHL(Packet *packet) {
    EV_INFO <<"!!! --> Ipv4_Service::handlePacketFromHL(Packet *packet)" << "\n"; // new added
...
// ^-^
void Ipv4_Service::datagramLocalOut(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_Service::datagramLocalOut(Packet *packet)\n"; // new added
 ...
 */
const InterfaceEntry *Ipv4_Service::determineOutgoingInterfaceForMulticastDatagram(const Ptr<const Ipv4Header>& ipv4Header, const InterfaceEntry *multicastIFOption)
{
...
// ^-^
void Ipv4_Service::routeUnicastPacket(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_Service::routeUnicastPacket(Packet *packet)\n"; // new added
...
// ^-^
void Ipv4_Service::routeUnicastPacketFinish(Packet *packet) {
    EV_INFO <<"\n!!! --> Ipv4_Service::routeUnicastPacketFinish(Packet *packet) \n"; // new added
    EV_INFO << "output interface = " << getDestInterface(packet)->getInterfaceName() << ", next hop address = " << getNextHop(packet) << "\n";
    numForwarded++;
    fragmentPostRouting(packet);
}

void Ipv4_Service::routeLocalBroadcastPacket(Packet *packet)
{
...
const InterfaceEntry *Ipv4_Service::getShortestPathInterfaceToSource(const Ptr<const Ipv4Header>& ipv4Header) const
{
    return rt->getInterfaceForDestAddr(ipv4Header->getSrcAddress());
}

void Ipv4_Service::forwardMulticastPacket(Packet *packet)
{
    const InterfaceEntry *fromIE = ift->getInterfaceById(packet->getTag<InterfaceInd>()->getInterfaceId());
    const auto& ipv4Header = packet->peekAtFront<Ipv4Header>();
...
// ^-^
void Ipv4_Service::reassembleAndDeliver(Packet *packet) {
    EV_INFO << "!!! --> Ipv4_Service::reassembleAndDeliver(Packet *packet) \n"; // new added
    EV_INFO << "Delivering " << packet << " locally.\n";
...
// ^-^
void Ipv4_Service::reassembleAndDeliverFinish(Packet *packet) {
    EV_INFO << "!!! --> Ipv4_Service::reassembleAndDeliverFinish(Packet *packet) \n"; // new added
...
// ^-^ 拆包 !!!
void Ipv4_Service::decapsulate(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_Service::decapsulate(Packet *packet) \n";
...
// ^-^
void Ipv4_Service::fragmentPostRouting(Packet *packet) {
    EV_INFO<<"!!! --> Ipv4_Service::fragmentPostRouting(Packet *packet) \n"; // new added
...
void Ipv4_Service::setComputedCrc(Ptr<Ipv4Header>& ipv4Header)
{
...
void Ipv4_Service::insertCrc(const Ptr<Ipv4Header>& ipv4Header)
{
...
// ^-^
void Ipv4_Service::fragmentAndSend(Packet *packet) {
    EV_INFO<<"\n!!! --> Ipv4_Service::fragmentAndSend(Packet *packet) \n";  // new added
    const InterfaceEntry *destIE = ift->getInterfaceById(packet->getTag<InterfaceReq>()->getInterfaceId());
    Ipv4Address nextHopAddr = getNextHop(packet);
...
// ^-^  IP封包 !!!
void Ipv4_Service::encapsulate(Packet *transportPacket) {
    EV_INFO<<"!!! --> Ipv4_Service::encapsulate(Packet *transportPacket) \n"; // new added
...
// ^-^
void Ipv4_Service::sendDatagramToOutput(Packet *packet) {
    EV_INFO << "!!! --> Ipv4_Service::sendDatagramToOutput(Packet *packet) \n"; // new added
...
void Ipv4_Service::arpResolutionCompleted(IArp::Notification *entry)
{
...
void Ipv4_Service::arpResolutionTimedOut(IArp::Notification *entry)
{
    if (entry->l3Address.getType() != L3Address::IPv4)
        return;
    auto it = pendingPackets.find(entry->l3Address.toIpv4());
...
MacAddress Ipv4_Service::resolveNextHopMacAddress(cPacket *packet, Ipv4Address nextHopAddr, const InterfaceEntry *destIE)
{
...
// ^-^
void Ipv4_Service::sendPacketToNIC(Packet *packet) {
    EV_INFO << "!!! --> Ipv4_Service::sendPacketToNIC(Packet *packet) \n";
...
// NetFilter:
void Ipv4_Service::registerHook(int priority, INetfilter::IHook *hook)
{
    Enter_Method("registerHook()");
    NetfilterBase::registerHook(priority, hook);
}

void Ipv4_Service::unregisterHook(INetfilter::IHook *hook)
{
    Enter_Method("unregisterHook()");
    NetfilterBase::unregisterHook(hook);
}

void Ipv4_Service::dropQueuedDatagram(const Packet *packet)
{
...
void Ipv4_Service::reinjectQueuedDatagram(const Packet *packet)
{
...
// ^-^
INetfilter::IHook::Result Ipv4_Service::datagramPreRoutingHook(Packet *packet) {
    EV_INFO<<"    --> Ipv4_Service::datagramPreRoutingHook(Packet *packet) \n"; // new added
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result Ipv4_Service::datagramForwardHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
// ^-^
INetfilter::IHook::Result Ipv4_Service::datagramPostRoutingHook(Packet *packet) {
    EV_INFO<<"    --> Ipv4_Service::datagramPostRoutingHook(Packet *packet)";  //new added
    for (auto & elem : hooks) {
...
void Ipv4_Service::handleStartOperation(LifecycleOperation *operation)
{
    start();
}

void Ipv4_Service::handleStopOperation(LifecycleOperation *operation)
{
    // TODO: stop should send and wait pending packets
    stop();
}

void Ipv4_Service::handleCrashOperation(LifecycleOperation *operation)
{
    stop();
}

void Ipv4_Service::start()
{
}

void Ipv4_Service::stop()
{
    flush();
...
INetfilter::IHook::Result Ipv4_Service::datagramLocalInHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
// ^-^
INetfilter::IHook::Result Ipv4_Service::datagramLocalOutHook(Packet *packet) {
...
void Ipv4_Service::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    Enter_Method_Silent();
...
void Ipv4_Service::sendIcmpError(Packet *origPacket, int inputInterfaceId, IcmpType type, IcmpCode code)
{
