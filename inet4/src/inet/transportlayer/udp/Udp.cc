...
Udp::SockDesc *Udp::createSocket(int sockId, const L3Address& localAddr, int localPort) {
    EV_INFO << "Udp::createSocket(int sockId, const L3Address& localAddr, int localPort)"<< "\n"; // new added
...
void Udp::handleUpperPacket(Packet *packet) {
    EV_INFO << "Udp::handleUpperPacket(Packet *packet)"<< "\n"; // new added
    if (packet->getKind() != UDP_C_DATA) throw cRuntimeError("Unknown packet command code (message kind) %d received from app", packet->getKind());
    ...
    if (srcAddr.isUnspecified()) addressReq->setSrcAddress(srcAddr = sd->localAddr);
    if (destAddr.isUnspecified()) addressReq->setDestAddress(destAddr = sd->remoteAddr);
    ...
    if (srcPort == -1) srcPort = sd->localPort;
    if (destPort == -1) destPort = sd->remotePort;
    ...
    if (interfaceReq == nullptr && destAddr.isMulticast()) {
        ...
        if (interfaceId != -1) packet->addTagIfAbsent<InterfaceReq>()->setInterfaceId(interfaceId);
    }

    if (addressReq->getDestAddress().isUnspecified()) throw cRuntimeError("send: unspecified destination address");
    if (destPort <= 0 || destPort > 65535) throw cRuntimeError("send: invalid remote port number %d", destPort);
    if (packet->findTag<MulticastReq>() == nullptr) packet->addTag<MulticastReq>()->setMulticastLoop(sd->multicastLoop);
    if (sd->ttl != -1 && packet->findTag<HopLimitReq>() == nullptr) packet->addTag<HopLimitReq>()->setHopLimit(sd->ttl);
    if (sd->dscp != -1 && packet->findTag<DscpReq>() == nullptr) packet->addTag<DscpReq>()->setDifferentiatedServicesCodePoint(sd->dscp);
    if (sd->tos != -1 && packet->findTag<TosReq>() == nullptr) {
        packet->addTag<TosReq>()->setTos(sd->tos);
        if (packet->findTag<DscpReq>()) throw cRuntimeError("setting error: TOS and DSCP found together");
    }
    ...
    if (destAddr.getType() == L3Address::IPv4) l3Protocol = &Protocol::ipv4;
    else if (destAddr.getType() == L3Address::IPv6) l3Protocol = &Protocol::ipv6;
    else l3Protocol = &Protocol::nextHopForwarding;
    ...
    if(totalLength.get() > UDP_MAX_MESSAGE_SIZE) throw cRuntimeError("send: total UDP message size exceeds %u", UDP_MAX_MESSAGE_SIZE);
